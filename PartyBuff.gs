 ...
 */
 const skills = [
  ...
  const ntimes = scaleToFiveMinutes(numberBasedOnCost(mana, skill.cost), DELAY);
  
  Logger.log("Casting, "+skill.description+", "+ntimes+" times with available mana("+mana+")");
  repeat(function(){ PLAYER.cast(skill.name); }, ntimes, DELAY);
}